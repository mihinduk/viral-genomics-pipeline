...
# Use KNOWN_VIRUSES for consistency with mutations script
...
print("Using KNOWN_VIRUSES for gene coordinates (consistent with mutations script)")
try:
    # Keep VCM available as fallback but don't use it for gene coordinates
...
# Try to load virus configurations
KNOWN_VIRUSES = {}
try:
...
def create_depth_plot(depth_df, accession, title=None, min_depth_threshold=200, log_scale=True, 
    ...
    for gene, (start, end) in gene_coords.items():
        # Skip NS1' in depth coloring - it's only shown as hatching
        if gene == "NS1'" or gene == "nonstructural_protein_NS1_prime":
            continue
    ...
    for handle, label in zip(sorted_handles, sorted_labels):
        ...
        if original_gene:
            # Check if this is NS1' - the ONLY gene that should appear as transparent hatched in legend
            is_frameshift = (original_gene == "NS1'" or original_gene == "nonstructural_protein_NS1_prime" or 
                           label == "NS1'" or "NS1'" in original_gene)
            
            # Debug: Print legend frameshift detection
            if "'" in original_gene or "prime" in original_gene.lower():
                print(f"DEBUG LEGEND: Gene '{original_gene}' (label: '{label}') - is_frameshift: {is_frameshift}")
    ...
    for gene, (start, end) in gene_coords.items():
        ...
        # Check if this is NS1' - the ONLY frameshift gene that should have transparent hatching
        is_frameshift = (gene == "NS1'" or gene == "nonstructural_protein_NS1_prime" or 
                        display_name == "NS1'" or "NS1'" in gene)
        
        # Debug: Print frameshift detection for NS1'
        if "'" in gene or "prime" in gene.lower() or is_frameshift:
            print(f"DEBUG: Gene '{gene}' (display: '{display_name}') - is_frameshift: {is_frameshift}")
        ...
        if is_frameshift and gene in overlapping_genes:
            ...
            # Regular gene - check for hatching (gene_coords already uses standard names)
            info = KNOWN_VIRUSES.get(accession, {})
            gene_properties = info.get("gene_properties", {})
            props = gene_properties.get(gene, {})  # gene is already a standard name
            should_hatch = props.get("hatching", False)
            hatch_pattern = props.get("hatch_pattern", "...") if should_hatch else None
            
            print(f"DEBUG: Gene '{gene}' -> hatching: {should_hatch} (pattern: {hatch_pattern})")
