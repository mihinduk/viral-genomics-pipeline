...
WORK_DIR="/ref/sahlab/data/microbes_db_build"
FINAL_DB_DIR="/ref/sahlab/data/microbes_db"
THREADS=4
...
declare -A GENOMES=(
    ["Mycoplasma_hyorhinis"]="https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=nuccore&id=CP003231.1&rettype=fasta&retmode=text"
...
for organism in "${!GENOMES[@]}"; do
    ...
    if [[ "${GENOMES[$organism]}" == *"eutils.ncbi.nlm.nih.gov"* ]]; then
        # Direct FASTA download, no compression
        if download_with_retry "${GENOMES[$organism]}" "${organism}.fna"; then
            if [ -f "${organism}.fna" ]; then
                echo "  ✓ Downloaded ${organism}.fna"
                DOWNLOADED_FILES+=("${organism}.fna")
            else
                echo "  ✗ Failed to download ${organism}.fna"
            fi
        else
...
makeblastdb -in "${DB_NAME}_combined.fna" \
            ...
            -title "Microbial Contamination Database" \
            -parse_seqids
...
echo ""
echo "========================================="
echo "Microbial database build completed!"
echo "========================================="
